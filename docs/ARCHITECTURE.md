...
End users never grant tokens. The bot uses one set of workspace tokens (admin-managed) to look up any teammate's _public_ activity. LinkedIn and personal email are the only sources that require per-user OAuth — they're treated as opt-in enrichment, never blocking.
